...
export default function AddGroupMembers({
  ...
  const submit = async (e?: React.FormEvent) => {
    ...
    setError(null);
    if (selected.length === 0) return setError('Select at least one user to add');
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/groups/${group._id}/add-members`, {
        method: 'PUT',
  ...
  return (
    <div className="p-4">
      ...
      <div className="mt-3">
        <button onClick={submit} disabled={loading} className="rounded bg-accent px-3 py-2 text-sm text-white">
